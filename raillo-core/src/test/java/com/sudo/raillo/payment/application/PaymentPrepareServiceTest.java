...
class PaymentPrepareServiceTest {
	...
	void preparePayment_success() {
		...
		List<SeatReservation> seatReservations = List.of(
		...
		Reservation reservation = PendingBookingFixture.builder()
			...
			.withSeatReservations(seatReservations)
			.withTotalFare(BigDecimal.valueOf(50000))
			.build();
		bookingRedisRepository.savePendingBooking(reservation);

		PaymentPrepareCommand request = new PaymentPrepareCommand(List.of(reservation.getId()));
		...
		assertThat(result.totalAmount()).isEqualByComparingTo(reservation.getTotalFare());
	}
	...
	void preparePayment_multiplePendingBookingsWithMultipleSeats_success() {
		...
		Reservation reservation1 = PendingBookingFixture.builder()
			...
			.withSeatReservations(List.of(
				new SeatReservation(seatIds.get(0), PassengerType.ADULT),
				new SeatReservation(seatIds.get(1), PassengerType.CHILD)
			))
			.build();
		bookingRedisRepository.savePendingBooking(reservation1);
		...
		Reservation reservation2 = PendingBookingFixture.builder()
			...
			.withArrivalStopId(arrivalStop.getId())
			.withSeatReservations(List.of(
				new SeatReservation(seatIds.get(2), PassengerType.ADULT),
				new SeatReservation(seatIds.get(3), PassengerType.SENIOR)
			))
			.build();
		bookingRedisRepository.savePendingBooking(reservation2);
	...
	void preparePayment_accessDenied_throwsException() {
		...
		Reservation othersReservation = PendingBookingFixture.builder()
			...
			.withTotalFare(BigDecimal.valueOf(50000))
			.build();
		bookingRedisRepository.savePendingBooking(othersReservation);

		PaymentPrepareCommand request = new PaymentPrepareCommand(List.of(othersReservation.getId()));
